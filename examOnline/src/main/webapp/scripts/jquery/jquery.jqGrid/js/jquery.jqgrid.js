...
$.fn.jqGrid = function( pin )
{
	...
	return this.each( function() {
		...
		$(grid.bDiv)
			...
			.append($('<div style="position:relative;'+(isMSIE && $.browser.version < 8 ? "" : "")+'"></div>').append('<div></div>').append(this))
			...
			case "custom" :
				...
				try {
					if($.isFunction(options.custom_element)) {
                        //修改自定义函数返回对象，判断编辑类型
                        var aa =   options.custom_element.call(this,vl,options);
						var celm = aa.el;
						if(celm) {
                            ...
                            switch (aa.type){
                                case "datepicker":
                                      $(celm).datepicker(options);
                                      break;
                                case "timepicker":
                                      $(celm).timepicker(options);
                                      break;
                            };
...
// Date Validation Javascript
	...
	//增加校验格式 hh:mm:ss 和 hh:mm:ss am/pm
       var re = /^(\d{2}):(\d{2})([ap]m)?$|^(\d{2}):(\d{2}):(\d{2})\s*([ap]m)?$/,regs;
	...
	checkValues : function(val, valref,g, customobject, nam) {
		...
		if(edtrul) {
			...
			if(edtrul.time === true) {
				if( !(rqfield === false && this.isEmpty(val)) ) {
					if(!this.checkTime (val)) { return [false,nm+": "+$.jgrid.edit.msg.date+" - hh:mm:ss or hh:mm:ss (am/pm)",""]; }
				}
			}
			if(edtrul.url === true) {
				if( !(rqfield === false && this.isEmpty(val)) ) {
...
$.jgrid.extend({
    ...
    getJsontosubmit:function(custompara){
            var  insertrow=[] , editrow=[], delrow=[],$t= this,jsonText,isid=false,
                 targetjson = {addRows:"",updateRows:"",deleteRows:""},
                  //获取编辑的数据
                 editrowobj =  jQuery($t).jqGrid('getChangedCells','all');
            var delids =  $t[0].p.delrowid,
                 newidrule = "newrow";
            for(var i=0;i<$t[0].p.colModel.length;i++){
                if($t[0].p.colModel[i].name == "id") {
                isid  =   true ;
                break;
                 }
             };
    ...
    //调整工具栏布局
    navGridposition :function(){
         var  $t= this,opt=this[0].p;
        var topPagerDiv = $(opt.toppager)[0];
        $(opt.toppager+"_"+opt.pagerpos, topPagerDiv).remove();
        $(opt.toppager+"_"+opt.recordpos, topPagerDiv).remove();
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
       ...
       //获取主键   
       for(var i=0;i<that[0].p.colModel.length;i++){
          if(that[0].p.colModel[i].key == true) {
              key  =   that[0].p.colModel[i].name ;
              break;
           }
       };
       ...
       function delrow(id){
         //拷贝数组
         that[0].p.delrowid = id.concat(that[0].p.delrowid);
         //提交删除数据时，按key值提交
         /*
         var delids = that[0].p.delrowid ;
            for(var j=0;j<delids.length;j++){
               var delid = delids[j];
               for(var i=0;i<that[0].p.data.length;i++){
               if(that[0].p.data[i].id == delid) {
                   delids[j]  =   that[0].p.data[i][key] ;
                   break;
               }
             };
            };
         that[0].p.delrowid = delids ;
         */
      ...
      function addrow(){
        ...
        //根据列名组织新增行数据
        //newrowdata["id"] = newid ;
	    $(that).jqGrid('addRowData',newid,newrowdata ,'last');
        serial++;
      };
		...
		return this.each(function() {
			...
			for(var i = 0; i<clone; i++) {
				...
				if (o.add) {
					...
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.addicon+"'></span><span class='ui-pg-button-text'>"+o.addtext+"</span></div>");
				...
				if (o.edit) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					pEdit = pEdit || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.editicon+"'></span><span class='ui-pg-button-text'>"+o.edittext+"</span></div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.view) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					pView = pView || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.viewicon+"'></span><span class='ui-pg-button-text'>"+o.viewtext+"</span></div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.del) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					pDel = pDel || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.delicon+"'></span><span class='ui-pg-button-text'>"+o.deltext+"</span></div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.search) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					pSearch = pSearch || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.searchicon+"'></span><span class='ui-pg-button-text'>"+o.searchtext+"</span></div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.refresh) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.refreshicon+"'></span><span class='ui-pg-button-text'>"+o.refreshtext+"</span></div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
	...
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			...
			var findnav, $t = this;
            //当toppager为true时在top工具栏增加按钮
            if($t.p.toppager){
                findnav = $("#"+$t.p.id+"_toppager .navtable")[0];
            }else{
                findnav = $(".navtable",elem)[0] ;
            };
			if (findnav) {
				...
				if(p.buttonicon.toString().toUpperCase() == "NONE") {
					...
					$(tbd).addClass('ui-pg-button ui-corner-all').append("<div class='ui-pg-div'><span class='ui-icon "+p.buttonicon+"'></span><span class='ui-pg-button-text'>"+p.caption+"</span></div>");
				}
...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if ($t.p.colModel[iCol].editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				// $("input, select, textarea",cc).focusout(function() {$($t).jqGrid("saveCell",iRow,iCol);});  //失去焦点保存cell 跟日期组件选择日期有冲突
           ...
           // if( $.isFunction(oneditfunc)) { oneditfunc.call($t, iRow,iCol); }
	...
	GridNav : function() {
		return this.each(function () {
			...
			$t.p.knv = $t.p.id + "_kn";
			var selection = $("<span style='width:0px;height:0px;background-color:black;' tabindex='0'><span tabindex='-1' id='"+$t.p.knv+"'></span></span>"),
			i, kdir;
			...
			$($t.rows).each(function(j){
				...
				if ($(this).hasClass("edited")) {
					$('td',this).each( function(i) {
                        ...
                        value = $(this).attr("value") ;//select编辑后的value值
						if ( nm !== 'cb' && nm !== 'subgrid') {
							if (mthd=='dirty') {
								...
								try {
								      //如果编辑类型为select、单元格编辑过、没有设置反格式化函数则获取编辑后的value值
								      if(et=="select"  &&  typeof(value)!="undefined" && typeof(uf)=="undefined" ){
                                          res[nm] = value;
                                      } else{
									      res[nm] = $.unformat(this,{rowId:$t.rows[j].id,colModel:$t.p.colModel[i]},i);
									  }
...
function datepicker (value, options) {
     var result={}, el = document.createElement("input");
      el.type="text";
      el.value = value;
      result.el =  el ;
